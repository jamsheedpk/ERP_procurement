...
router.get("/", async (req, res) => {
  try {
    ...
    const [
      ...
      Employee.find({ status: { $ne: "inactive" } }).select("empId name status dept deptId grade av"),
      Department.find().sort({ name: 1 }),
      LeaveRequest.find({ status: "pending" }).sort({ submitted: -1 }),
      Renewal.find().sort({ days: 1 }),
      Opening.find({ status: { $ne: "closed" } }).sort({ posted: -1 }),
      PayrollRun.findOne().sort({ createdAt: -1 }),
      Activity.find().sort({ createdAt: -1 }).limit(7),
      HeadcountSnapshot.find().sort({ order: 1 }),
      AttendanceDay.findOne().sort({ date: -1 }),
